...
previous_channel_list_length = 0
# Command Management -----------------------------------------------------------
def manage_user_input(user: IRC.User):
    global previous_channel_list_length
    if previous_channel_list_length < len(IRC.channels):
        for channel in IRC.channels:
            module_tools.update_modules_for_new_channel(channel)
        previous_channel_list_length = len(IRC.channels)
    ...
    if not module_tools.moderation_check(user):
        ...
        if DEFAULT_COMMAND_PREFIX in user.command[0]:
            # Blind-fire the user into the general commands to see if they tried
            # to run a custom channel-specific command.
            module_tools.run_general_command(user)
...
def cleanup():
    module_tools.run_shutdown_functions()
